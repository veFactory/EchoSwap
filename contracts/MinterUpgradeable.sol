...
contract MinterUpgradeable is IMinter, OwnableUpgradeable {
    ...
    uint public weekly; // represents a starting weekly emission of 2.6M ECHO (ECHO has 18 decimals)
    ...
    function initialize(    
        ...
        weekly = 2_600_000 * 1e18; // represents a starting weekly emission of 2.6M ECHO (ECHO has 18 decimals)
        isFirstMint = true;
    ...
    function circulating_supply() public view returns (uint) {
        return _echo.totalSupply() - _echo.balanceOf(address(_ve));
    }
    ...
    function calculate_rebate(uint _weeklyMint) public view returns (uint) {
        uint _veTotal = _echo.balanceOf(address(_ve));
    ...
    function update_period() external returns (uint) {
        ...
        if (block.timestamp >= _period + WEEK && _initializer == address(0)) { // only trigger if new week
            ...
            _echo.approve(address(_voter), _gauge);
